...
public:
    ...
    NN(string& method, initializer_list<int> param) {
        if (method != "classification") {
            cout << "Method '" << method << "' does not exist!" << endl;
            abort();
        }
    ...
    void set_config(string& method, vector<int> param) {
        if (method != "classification") {
            cout << "Method '" << method << "' does not exist!" << endl;
            abort();
        }
    ...
    void train() {
        for (int ep = 0; ep < NUM_EPOCHS; ep++) {
            ...
            for (int i = 0; i < dataset.size() / BATCH_SIZE; i++) {
                ...
                matrix<Type> x = batch[0][0];
                matrix<Type> y = batch[0][1];

                matrix<Type> z = forward(x);
...
private:
    ...
    void build() {
        weights.load_size((int) layers.size()-1, 2);
        neurons.load_size((int) weights.size(), 2);
        for (int i = 0; i < layers.size()-1; i++) {
            matrix<Type> W = randn(layers[i], layers[i + 1]) - 0.5;
            matrix<Type> b = randn(1, layers[i + 1]) - 0.5;
            W *= 2 * sqrt(1. / layers[i]);
            b *= 2 * sqrt(1. / layers[i]);
